            ...
            <div class="row mt-5">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Stock Return Analysis By Branch & Item</li>
                    </ol>
                </nav>
            </div>
            <div class="row mt-2">
                <div class="card col-md-12" style="box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
                    <div class="card-body">
                        <form method="get" action="stockreturnbyi.jsp" >
                            <div class="row">
                                <div class="col-md-4">
                            ...
                            <tbody>
                                <%
                                    if(Date != null && Date2 != null){
                                        ...
                                        List<StockReturn> IR=cr.list();
                                      
                                    for (StockReturn r : IR) {
                                        Set<ReturnItem> II=r.getReturnItems(); 
                                        for(ReturnItem ir: II){
                                       
                                        
                                        
                                        if(branch > 0 && account ==0){
                                            if(ir.getBranch().getIdbranch()  == branch ){
                                                total_sales=total_sales + ir.getAmount();
                                                total_qty=total_qty+ir.getQtt();
                                                %>
                                                    <tr>
                                                        <td><%=ir.getStockReturn().getIdstockReturn() %></td>
                                                        <td><%=ir.getStock().getItems().getName() %></td>
                                                       <%
                                                            if (ir.getStockReturn().getStatus() == 1) {
                                                        %>
                                                        <td><span class="badge bg-info">Pending</span></td>
                                                        <%
                                                        } else if(ir.getStockReturn().getStatus()==2) {
                                                        %>
                                                        ...
                                                        %>
                                                        <td><%= ir.getAmount() %></td>
                                                        <td><%= ir.getQtt() %> ( <%=ir.getStock().getItems().getType().getName() %> )</td>
                                                        <td><%= ir.getStockReturn().getUser().getName() %></td>
                                                        <td><%= ir.getBranch().getName() %></td>
                                            ...
                                            if(ir.getStock().getItems().getItemId() == account){
                                                total_sales=total_sales + ir.getAmount();
                                                total_qty=total_qty+ir.getQtt();
                                                %>
                                                    <tr>
                                            ...
                                            if(ir.getStock().getItems().getItemId() == account && ir.getBranch().getIdbranch()  == branch ){
