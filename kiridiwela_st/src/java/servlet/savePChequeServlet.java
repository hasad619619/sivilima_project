 ...
 */
@WebServlet(name = "savePChequeServlet", urlPatterns = {"/savePChequeServlet"})
public class savePChequeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (request.getParameter("desc").equals("")
                    || request.getParameter("po").equals("")
                    || request.getParameter("ret").equals("")
                    || request.getSession().getAttribute("p1") == null
                    || request.getSession().getAttribute("invp") == null
                    || request.getParameter("payment").equals("")
                ...
                if (invoice.size() > 0) {
                    ...
                    PurchusInvoiceRecipt PIR3 = new PurchusInvoiceRecipt();
                    ...
                    PIR3.setBranch(BRANCH);
                    PIR3.setUser(Usser);
                    PIR3.setRemarks("PERCHASE");
                    PIR3.setChequeBranch(request.getParameter("branch"));
                    PIR3.setAccountBySupplier(Suppp_Account);
                    PIR3.setAccountByBank(BankAcc);
                    PIR3.setPaymentType(pay_type);
                    PIR3.setPurchusInvoice(PI);
