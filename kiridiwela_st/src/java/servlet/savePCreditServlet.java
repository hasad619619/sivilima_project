 ...
 */
@WebServlet(name = "savePCreditServlet", urlPatterns = {"/savePCreditServlet"})
public class savePCreditServlet extends HttpServlet {
        ...
        try {
            if (request.getParameter("desc").equals("") || request.getParameter("po").equals("") || request.getParameter("ret").equals("") || request.getSession().getAttribute("p1") == null || request.getSession().getAttribute("invp") == null) {
                ...
                if (invoice.size() > 0) {
                    ...
                    double apita_denna = Suppp_Account.getCreditBalance();
                    double api_denna = Suppp_Account.getDebitBalance();

                    if (apita_denna == 0.0) {
                        Suppp_Account.setDebitBalance(payble + api_denna);
                    } else {

                        double newval = payble + api_denna;
                        if (newval <= apita_denna) {
                            Suppp_Account.setCreditBalance(apita_denna - newval);
                            Suppp_Account.setDebitBalance(0.0);
                        } else if (newval > apita_denna) {
                            Suppp_Account.setDebitBalance(newval - apita_denna);
                            Suppp_Account.setCreditBalance(0.0);

                        }
                    }

                    ses.update(Suppp_Account);
                    ses.save(PI);
