...
<html lang="en">
        ...
        <main role="main" class="container">
            ...
            <div class="row mt-5">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" style="font-size: 16px;" aria-current="page">Sub Category</li>
                    </ol>
                </nav>
            </div>
            ...
            <div class="row mt-2">
                <div class="card col-md-12" style="box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);" >
                    ...
                    <div class="card-body">
                        <table id="table1" class="table table-active">

                            <thead>
                                <tr style="background-color: #10ac84 !important; color: white !important; ">
                                    <th id="dev_id" class="dev_id" >Sub Category ID</th>
                                    <th>name</th>
                                    <th>Category</th>
                                    <th>status</th>
                                    <th>option</th>
                            ...
                            <tbody>
                                <%
                                    ...
                                    Criteria cr = ses.createCriteria(SubCategory.class);
                                    List<SubCategory> de = cr.list();
                                    
                                    
                                    if (de.size() > 0) {
                                        for ( SubCategory DE : de) {


                                %>
                                <tr>
                                    <td><%=DE.getSubcategoryId()%></td>
                                    <td><%=DE.getName() %></td>
                                    <td><%=DE.getCategory().getName() %></td>
                                    ...
                                    <td>
                                        <button class="btn btn-warning btn-sm delete_dev" id="delete_dev"  value="<%=DE.getSubcategoryId() %>">Delete | Reset</button>
                                        ...
                                        <input type="hidden" name="u_cat" id="u_cat" class="u_cat" value= "<%= DE.getCategory().getIdcategory() %>"/>
                                        <button  class="btn btn-info btn-sm update_dev"  id="update_dev" value="<%=DE.getSubcategoryId()%>">Update</button>
                        ...
                        <form action="" method="post">
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            <select type="number" class="form-control"    name="ct" id="ct">
                                                    <%  
                                                        Session s = connection.getConnection.getSessionFactory().openSession();
                                                        Criteria cr5 = s.createCriteria(Category.class);
                                                        cr5.add(Restrictions.eq("status", 1));
                                                        List<Category> c5 = cr5.list();
                                                        if (c5.size() > 0) {
                                                            for (Category B : c5) {
                                                    %>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control" id="inputEmail3" placeholder="Sub Category name" name="ename" id="ename">
                                        </div>
                                    </div>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Update Sub Category Details</h5>
                            ...
                            <form action="" method="post">
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-10">
                                            <select type="number" class="form-control"    name="ct2" id="ct2">
                                                    <%  
                                                        Session s2 = connection.getConnection.getSessionFactory().openSession();
                                                        Criteria cr6 = s2.createCriteria(Category.class);
                                                        cr6.add(Restrictions.eq("status", 1));
                                                        List<Category> c6 = cr6.list();
                                                        if (c6.size() > 0) {
                                                            for (Category B : c6) {
                                                    %>
                                                    <option value="<%=B.getIdcategory()%>"><%=B.getName()%></option>
        ...
        <script type="text/javascript">
         ...
         var cat=$(this).closest('tr').find('input[name="u_cat"]').val();
         
         
         document.getElementById("col_1").value = ""+name;
         document.getElementById("col_4").value = ""+id;
         document.getElementById("ct2").value = ""+cat;
    ...
    $(".update_model").click(function () {
        ...
        swal({
            title: "Are you sure?",
            text: "do you want to update Sub Category details? ",
            icon: "warning",
            buttons: true,
            dangerMode: true,
            }).then((willDelete) => {
                    if (willDelete) {
                        ...
                        if (data === "1") {
                            ...
                            swal({
                                title: "Good job!",
                                text: "you update Sub Category from list !",
                                icon: "success",
                                button: "OK"
                            }).then((value) => {
                                    location.reload();
    ...
    $("#save_dev").click(function () {
        ...
        $.ajax({
            url: 'saveSubCategoryServlet',
            data: {
                name: $('input[name="ename"]').val(),
                ct: $('#ct').find(":selected").val()
